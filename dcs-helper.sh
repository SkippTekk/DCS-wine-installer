...
#Wine download URL
wine_download="https://github.com/starcitizen-lug/raw-wine/releases/download/10.3/raw-wine-10.3-stg.tar.gz"
wine_file="raw-wine-10.3-stg.tar.gz"

function option1 {
    echo "\033[32m Alright, so you wanted to installed DCS on wine... Lets get to it!" 
    ...
    install_dir="$(zenity --file-selection --directory --title="Choose your DCS install location" --filename="$HOME/" 2>/dev/null)"
        ...
        cd $install_dir 
        mkdir runner 
        tar -xf $script_dir/tmp/raw-wine-10.3-stg.tar.gz -C runner/
        

    export wine_path="$install_dir/runner/raw-wine-10.3-stg/bin"
    ...
    "$wine_path"/wine start "$script_dir/tmp/$dcs_installer" 
